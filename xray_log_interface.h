...
struct XRayLogImpl {
  ...
  enum XRayLogInitStatus (*log_init)(size_t, size_t, void *, size_t);

  /// The log finalization routine provided by the implementation.
  ///
  /// See XRayLogInitStatus for details on what the implementation MUST return
  /// when called.
  enum XRayLogInitStatus (*log_finalize)();
  ...
  /// when called.
  enum XRayLogFlushStatus (*flush_log)();
};
...
void __xray_set_log_impl(struct XRayLogImpl Impl);
...
///     implementation.
enum XRayLogRegisterStatus __xray_log_register_mode(const char *Mode,
                                               struct XRayLogImpl Impl);
...
enum XRayLogRegisterStatus __xray_log_select_mode(const char *Mode);
...
enum XRayLogInitStatus __xray_log_init(size_t BufferSize, size_t MaxBuffers,
...
enum XRayLogInitStatus __xray_log_init_mode(const char *Mode, const char *Config);
...
void __xray_log_set_buffer_iterator(struct XRayBuffer (*Iterator)(struct XRayBuffer));
...
///
enum XRayLogFlushStatus __xray_log_process_buffers(void (*Processor)(const char *,
                                                                XRayBuffer));
